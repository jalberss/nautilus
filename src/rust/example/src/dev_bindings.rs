...
pub struct dev_iterator {
    hop: usize,
    begin: *mut list_head,
    end: *const list_head,
}
extern {
    static dev_list: list_head;
}
impl dev_iterator { // 
    pub fn new() -> dev_iterator {
        dev_iterator {
            hop: containerof_field_offset!(nk_dev : dev_list_node),
            begin: unsafe {dev_list.next},
            end: unsafe {&dev_list}
        }
    }

    pub fn new_with_begin(begin: *mut list_head) -> dev_iterator {
        dev_iterator {
            hop: containerof_field_offset!(nk_dev : dev_list_node),
            begin: unsafe {begin},
            end: unsafe {&dev_list}
...
impl IntoIterator for nk_dev {
    type Item = *mut nk_dev;
    type IntoIter = dev_iterator;

    fn into_iter(mut self) -> Self::IntoIter {
        dev_iterator::new_with_begin(&mut self.dev_list_node)
    }
}

impl Iterator for dev_iterator { //in order to generalize further, we would need to know only where the thing begins and ends
    type Item = *mut nk_dev;

    fn next(&mut self) -> Option<Self::Item> {
        use libc::c_char;
        unsafe {
            let cur: *mut c_char = (self.begin as *mut c_char).sub(self.hop);
            if self.end == self.begin{
                return None;
            }
                
            let ret = Some(cur as *mut nk_dev);
...
pub const __clang_minor__: u32 = 9;
pub const __clang_patchlevel__: u32 = 1;
pub const __clang_version__: &'static [u8; 29usize] = b"3.9.1 (tags/RELEASE_391/rc2)\0";
...
pub const __VERSION__: &'static [u8; 52usize] =
    b"4.2.1 Compatible Clang 3.9.1 (tags/RELEASE_391/rc2)\0";
